...
const PortfolioAssetsList = () => {
  ...
  const { loading, portfolioCoin, removePortfolioCoin } = usePortfolio();

  const getCurrentBalance = () =>
    portfolioCoin.reduce(
      (total, currentAsset) =>
  ...
  const getCurrentValueChange = () => {
    ...
    const boughtBalance = portfolioCoin.reduce(
      (total, currentAsset) =>
  ...
  const onDeleteAsset = async (asset) => {
    console.log("hello");
    const newAssets = portfolioCoin.filter(
  ...
  const renderDeleteButton = (data) => {
    return (
      <Pressable
        ...
        onPress={() => {
          removePortfolioCoin(data.item.unique_id);
        }}
      >
